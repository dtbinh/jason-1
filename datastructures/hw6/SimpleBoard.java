...
public class SimpleBoard {
  ...
  public int hashCode() {
	...
	//For this question, I consulted the Wikipedia article http://en.wikipedia.org/wiki/Java_hashCode()
	//to determine h = 3*h+grid[x][y] (I got the idea from h = 31 * h + input.charAt(i); from the article)
	  
	int hashCode = 0;
	for (int i=0; i<DIMENSION; i++){
		for(int j=0; j<DIMENSION; j++){
			hashCode = 3*hashCode+this.grid[i][j];
		}
	}
