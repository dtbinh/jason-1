...
public class DList extends List {
  ...
  public DList() {
    ...
    //   the `list' field (second parameter) as well.
	  head = newNode(null, null, null, null); //The sentinel node should return null in any lists
	  head.next = head;
	  head.prev = head;
	  size = 0;
  }
  ...
  public void insertFront(Object item) {
    // Your solution here.  Similar to Homework 4, but now you need to specify
    //   the `list' field (second parameter) as well.
	  DListNode node = newNode(item, this, head, head.next);//"this" specifies the current list you are in
	  head.next = node;
	  if (head.prev == head){
		  head.prev = node;
	  }else{
  ...
  public void insertBack(Object item) {
    // Your solution here.  Similar to Homework 4, but now you need to specify
    //   the `list' field (second parameter) as well.
	  DListNode node = newNode(item, this, head.prev, head);//"this" specifies the current list you are in
	  head.prev = node;
	  if (head.next == head){
		  head.next = node;
	  }else{
